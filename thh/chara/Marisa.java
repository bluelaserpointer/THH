...
public class Marisa extends Chara{
	/**
	 * 
	 */
		...
		if(isActive){
			...
			if(this.detectMousePress_left()) {
				if(thh.isMouseOnImage(dodgeMarkerIID,dodgeMarkerX, dodgeMarkerY)) {
					...
					BulletInfo.kind = weaponSlot[slot];
					switch(weaponSlot[slot]){
						case MILLKY_WAY:
							BulletInfo.name = "MILLKY_WAY";
							BulletInfo.fastParamSet_ADSpd(mouseAngle,10,20);
						...
						case NARROW_SPARK:
							...
							BulletInfo.fastParamSet_XYADSpd(charaX,charaY,mouseAngle,0,thh.getImageByID(bulletIID[NARROW_SPARK]).getWidth(null));
							BulletInfo.accel = 1.0;
							BulletInfo.size = 50;
							BulletInfo.atk = 5;
							BulletInfo.offSet = 20;
						...
						case REUSE_BOMB:
							...
							BulletInfo.fastParamSet_ADSpd(mouseAngle,10,40);
